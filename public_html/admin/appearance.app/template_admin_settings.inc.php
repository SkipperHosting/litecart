<?php
  ...
  breadcrumbs::add(language::translate('title_template_admin', 'Admin Template'), document::link(WS_DIR_ADMIN, array('doc' => 'template'), array('app')));
  breadcrumbs::add(language::translate('title_template_admin_settings', 'Admin Template Settings'));

// Get template settings structure
  $settings = include vmod::check(FS_DIR_APP . 'includes/templates/' . settings::get('store_template_admin') .'/config.inc.php');
  if (!is_array($settings)) include vmod::check(FS_DIR_APP . 'includes/templates/' . settings::get('store_template_admin') .'/config.inc.php'); // Backwards compatibility

  if (empty($settings)) $settings = array();

// Insert template settings
  $saved_settings = json_decode(settings::get('store_template_admin_settings'), true);
  ...
  if (isset($_POST['save'])) {
    ...
    try {
      ...
      database::query(
        ...
        where `key` = '". database::input('store_template_admin_settings') ."'
        limit 1;"
      );
        ...
        <tbody>
          ...
          <tr>
            <td style="white-space: normal;">
              <u><?php echo language::translate(settings::get('store_template_admin').':title_'.$setting['key'], $setting['title']); ?></u><br />
              <?php echo language::translate(settings::get('store_template_admin').':description_'.$setting['key'], $setting['description']); ?>
            </td>
          ...
          <?php } else { ?>
          <tr>
            <td><?php echo language::translate(settings::get('store_template_admin').':title_'.$setting['key'], $setting['title']); ?></td>
            <td>
